...
export function makeThrottle(ms: number): (fn: () => void) => void {
  ...
  let lastAt = 0;
  return (fn) => {
    const delay = ms - (Date.now() - lastAt);
    if (delay <= 0) {
      clearTimeout(timer);
      timer = undefined;
      lastAt = Date.now();
...
// ---- Bat context with muted-amber match highlight ----

/**
 * Run bat on a file, showing ±contextLines around matchLines.
 * Matched text is highlighted with a muted amber background (not whole-line).
 * highlightRe must have the `g` flag.
 */
export function batContextHighlight(
  filePath: string,
  matchLines: number[],
  ...
  const min = Math.max(1, Math.min(...matchLines) - contextLines);
  const max = Math.max(...matchLines) + contextLines;
  const r = spawnSync(BAT_BIN, [
    '--color=always', '--style=numbers,header', '--paging=never', '--wrap=never',
    '--line-range', `${min}:${max}`,
    filePath,
  ], { encoding: 'utf-8', timeout: 5000 });
  if (!r || r.status !== 0 || !r.stdout) return null;

  const MATCH_BG = '\x1b[48;2;90;70;15m'; // muted amber
  const RESET_BG = '\x1b[49m';             // reset background only, keep foreground

  const lines = r.stdout.split('\n');
  const processed = lines.map(line => {
    if (!highlightRe) return line;
    const stripped = line.replace(/\x1b\[[0-9;]*m/g, '');
    const numMatch = stripped.match(/^\s*(\d+)\s/);
    if (!numMatch || !matchSet.has(parseInt(numMatch[1], 10))) return line;
    // Content starts after the first \x1b[0m (end of gutter color)
    const resetIdx = line.indexOf('\x1b[0m');
    if (resetIdx === -1) return line;
    const gutter = line.slice(0, resetIdx + 4);
    const content = line.slice(resetIdx + 4);
    highlightRe.lastIndex = 0;
    return gutter + content.replace(highlightRe, `${MATCH_BG}$&${RESET_BG}`);
  });
  return processed.join('\n').trimEnd();
}

// ---- Bat context with bold matched lines ----

/**
 * Run bat on a file, showing ±contextLines around matchLines.
 * Matched lines are bolded via ANSI rather than background-highlighted.
 */
export function batContextBold(filePath: string, matchLines: number[], contextLines = 3): string | null {
  if (matchLines.length === 0) return null;
  const matchSet = new Set(matchLines);
  const min = Math.max(1, Math.min(...matchLines) - contextLines);
  const max = Math.max(...matchLines) + contextLines;
  ...
  if (!r || r.status !== 0 || !r.stdout) return null;
  const lines = r.stdout.split('\n');
  const bolded = lines.map(line => {
    const stripped = line.replace(/\x1b\[[0-9;]*m/g, '');
    const numMatch = stripped.match(/^\s*(\d+)\s/);
    if (numMatch && matchSet.has(parseInt(numMatch[1], 10))) {
      // Re-apply bold after each SGR reset so bat's internal color codes don't cancel it
      const rebolded = line.replace(/(\x1b\[0m)/g, '$1\x1b[1m');
      return `\x1b[1m${rebolded}\x1b[0m`;
    }
    return line;
  });
  return bolded.join('\n').trimEnd();
